...
export class AgentRuntimeConversationElement extends LitElement {
  ...
  `;
    ...
    renderPermissionDialog(displayItems) {
        const permissions = this.state.pendingPermissions.filter((permission) => !this.hasRenderedToolAnchor(permission.tool_call_id, displayItems));
    ...
    renderDialogPermissionItem(permission) {
        const zh = this.locale.toLowerCase().startsWith('zh');
    ...
    hasRenderedToolAnchor(toolCallId, displayItems) {
        if (this.hideToolCalls)
            return false;
        const contentHasAnchor = (content) => Boolean(content?.split(/\r?\n/).some((line) => {
        ...
        return displayItems.some((item) => {
            ...
            return ((item.item.kind === 'assistant-markdown' || item.item.kind === 'notice') &&
