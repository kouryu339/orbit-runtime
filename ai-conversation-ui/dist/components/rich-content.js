...
import { parseConversationContent, toolCallForId, } from '../content/parser.js';
...
export class ConversationRichContentElement extends LitElement {
    static properties = {
        ...
        pendingPermissions: { attribute: false },
        permissionOperation: { type: String, attribute: 'permission-operation' },
        locale: { type: String },
        reveal: { type: Boolean },
    ...
    .tool-permission-arguments {
      ...
      background: var(--conversation-code-background);
      color: var(--conversation-text-muted);
      font: 11px/1.45 var(--conversation-font-mono);
      white-space: pre-wrap;
    }
  ...
  `;
    ...
    willUpdate(changed) {
        if (!changed.has('content') && !changed.has('reveal'))
            return;
        if (!this.reveal)
            return;
    ...
    render() {
        ...
        ></agent-mermaid-diagram>`;
            ...
            if (part.kind === 'tool') {
                ...
                const permission = this.permissionForTool(call.id);
                const busy = Boolean(permission && this.permissionOperation === permission.tool_call_id);
                const zh = this.locale.toLowerCase().startsWith('zh');
                const permissionArguments = permission
                    ? this.permissionArguments(permission)
                    : '';
                return html `<details
          class="part tool"
          part="tool-call"
          ...
          @toggle=${(event) => this.onToolToggle(call.id, event)}
        >
          <summary>
          ...
          ${call.detail
                    ...
                    : null}
          ${permission
                    ? html `<div class="tool-permission">
                <div class="tool-permission-note">
                  ${zh
                        ? '工具尚未执行，请确认是否允许这次调用。'
                        : 'The tool has not run yet. Confirm whether to allow this call.'}
                </div>
                ${permissionArguments
                        ? html `<pre class="tool-permission-arguments">${permissionArguments}</pre>`
                        : null}
                <div class="tool-permission-actions">
                  <button
                    ...
                    type="button"
                    ?disabled=${busy}
                    @click=${() => this.emitToolPermissionDecision(call.id, 'deny')}
                  >${zh ? '拒绝' : 'Deny'}</button>
                  <button
                    type="button"
                    ?disabled=${busy}
                    @click=${() => this.emitToolPermissionDecision(call.id, 'allow')}
    ...
    permissionForTool(callId) {
        return this.pendingPermissions.find((permission) => permission.tool_call_id === callId);
    }
    permissionArguments(permission) {
        const entries = Object.entries(permission.arguments ?? {});
        if (!entries.length)
            return '';
        return JSON.stringify(Object.fromEntries(entries.slice(0, 12)), null, 2);
    }
    emitToolPermissionDecision(toolCallId, decision) {
        this.dispatchEvent(new CustomEvent('agent-tool-permission-decision', {
            bubbles: true,
            composed: true,
            detail: { toolCallId, decision },
        }));
    }
    ...
    renderWidgetInput(widget, disabled) {
        const value = this.widgetValues[widget.raw] ?? '';
        if (widget.kind === 'select:single') {
            return html `<select
        ?disabled=${disabled}
        .value=${String(value)}
        @change=${(event) => this.setWidgetValue(widget, event.target.value)}
      >
        ...
        <label>
          <input
            ...
            ?disabled=${disabled}
            @change=${(event) => this.toggleMulti(widget, option, event)}
          >
          ${option}
        </label>
      `)}</div>`;
        ...
        <input
          ...
          .value=${String(value)}
          ?disabled=${disabled}
          @input=${(event) => this.setWidgetValue(widget, event.target.value)}
        >
    ...
    toggleMulti(widget, option, event) {
        const current = Array.isArray(this.widgetValues[widget.raw])
            ? this.widgetValues[widget.raw]
            : [];
    ...
    hasWidgetValue(widgets) {
        return widgets.some((widget) => {
            const value = this.widgetValues[widget.raw];
            return Array.isArray(value) ? value.length > 0 : Boolean(value);
        });
    }
    submitWidgets(widgets) {
        if (this.widgetSubmitted || this.widgetsExpired)
            return;
        const content = widgets
            .map((widget) => [widget.label, this.widgetValues[widget.raw]])
    ...
    async pickPath(widget) {
        ...
        if (path)
            this.setWidgetValue(widget, path);
    }
    ...
    emitRevealComplete() {
        if (this.completedContent === this.content)
            return;
...
if (!customElements.get('agent-conversation-rich-content')) {
    customElements.define('agent-conversation-rich-content', ConversationRichContentElement);
}
function sourceLength(part) {
    if (part.kind === 'mermaid')
        return part.source.length;
    if (part.kind === 'tool')
        return part.callId.length;
    if (part.kind === 'widgets') {
        return part.widgets.reduce((total, widget) => total + widget.raw.length, 0);
    }
    return part.content.length;
}
function revealBaseline(content) {
...
function nextRevealLength(content, current, step) {
