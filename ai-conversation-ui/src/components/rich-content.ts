...
export class ConversationRichContentElement extends LitElement {
  ...
  declare pendingPermissions: PendingToolPermission[];
  declare permissionOperation: string | null;
  ...
  protected override render() {
    ...
    return html`${parts.map((part, partIndex) => {
      ...
      if (part.kind === 'tool') {
        ...
        const busy = Boolean(
          permission && this.permissionOperation === permission.tool_call_id,
        );
  ...
  private permissionForTool(callId: string): PendingToolPermission | undefined {
    return this.pendingPermissions.find(
      (permission) => permission.tool_call_id === callId,
    );
  }
