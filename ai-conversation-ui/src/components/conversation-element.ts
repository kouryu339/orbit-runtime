...
import type {
  ...
  ConversationHostCapabilities,
  PresetMarkdown,
  BuiltinProviderCatalog,
  BuiltinProviderCatalogModel,
  BuiltinProviderCatalogProvider,
  ConversationProviderControlsPolicy,
  ProviderDefinitionsResult,
  ProviderModelDefinition,
  ConversationPersistenceEvent,
  ...
  implements AgentRuntimeConversationPublicApi
{
  ...
  static styles = css`
    ...
    * { box-sizing: border-box; }
    ...
    .permission-backdrop {
      ...
      place-items: center;
      padding: 18px;
      background: color-mix(in srgb, var(--conversation-surface) 72%, transparent);
      backdrop-filter: blur(3px);
    }
    .permission-dialog {
      width: min(100%, 520px);
    ...
    .permission-summary {
      margin: 8px 0 0;
      color: var(--conversation-text-muted);
      font: 12px/1.55 var(--conversation-font-body);
    }
    .permission-tools {
      display: grid;
      gap: 10px;
    ...
    .permission-tool {
      ...
      border: 1px solid var(--conversation-border);
      border-radius: 8px;
      background: color-mix(in srgb, var(--conversation-surface-raised) 82%, var(--conversation-surface));
    }
  ...
  declare transport: ConversationTransport | null;
  declare capabilities: ConversationHostCapabilities;
  declare presentationItems: readonly ConversationPresentationItem[];
  declare extensions: readonly ConversationExtension[];
  declare conversationId: string | null;
  declare locale: string;
  declare colorScheme: 'light' | 'dark' | 'system';
  declare theme: ConversationThemeName;
  declare density: ConversationDensity;
  declare persistence: ConversationPersistencePolicy;
  declare providerControls: ConversationProviderControlsPolicy;
  declare progressiveReveal: boolean;
  declare hideToolCalls: boolean;
  declare disclaimer: string;
  declare state: ConversationState;
  declare private draft: string;
  declare private historyOpen: boolean;
  declare private persistedConversations: PersistedConversation[];
  declare private persistenceOperation: string | null;
  declare private providerPanelOpen: boolean;
  declare private providerEditorOpen: boolean;
  declare private providerDefinitions: ProviderDefinitionsResult | null;
  declare private builtinProviderCatalog: BuiltinProviderCatalog | null;
  declare private providerEditorProviderUid: number | null;
  declare private providerEditorBuiltinProviderId: string;
  declare private providerEditorProviderName: string;
  declare private providerEditorBaseUrl: string;
  declare private providerEditorApiParadigm: string;
  declare private providerEditorApiKey: string;
  ...
  private providerEditorModelSequence = 0;
  private localPresetSequence = 0;
  private stickToBottom = true;
  private readonly completedRevealKeys = new Set<string>();
  ...
  async saveConversation(): Promise<PersistedConversation | null> {
    if (!this.persistence.enabled || !this.state.conversationId) return null;
  ...
  async restoreConversation(archiveId: string): Promise<void> {
    if (!this.persistence.enabled) return;
  ...
  async refreshConversationHistory(): Promise<void> {
    if (!this.persistence.enabled) return;
  ...
  private onToolPermissionDecision = (
    event: CustomEvent<{ toolCallId: string; decision: 'allow' | 'deny' }>,
  ): void => {
    event.stopPropagation();
    void this.resolveToolPermission(event.detail.toolCallId, event.detail.decision);
  };
  ...
  insertPresetMarkdown(preset: PresetMarkdown): string {
    const id =
      preset.id ?? `preset-${Date.now()}-${++this.localPresetSequence}`;
  ...
  updatePresentationItem(id: string, patch: PresentationItemPatch): void {
    this.presentationItems = this.presentationItems.map((item) =>
  ...
  protected override willUpdate(changed: Map<PropertyKey, unknown>): void {
    ...
    if (changed.has('persistence')) this.configurePersistenceController();
    if (changed.has('providerControls')) {
      ...
      if (this.providerControls.enabled) void this.refreshProviderDefinitions();
    }
  }
  ...
  protected override render() {
    ...
    return html`<section class="shell" part="shell">
      ...
      ${this.renderPermissionDialog(displayItems)}
  ...
  private renderPersistenceActions() {
    if (!this.persistence.enabled) return nothing;
  ...
  private renderProviderActions() {
    if (!this.providerControls.enabled) return nothing;
    ...
    const zh = this.locale.toLowerCase().startsWith('zh');
    if (!showModelSwitcher && !showImport) return nothing;
        ...
        ? html`<button
            ...
            @click=${() => {
              ...
              if (this.providerPanelOpen) {
                this.historyOpen = false;
                if (!this.providerDefinitions) void this.refreshProviderDefinitions();
              }
            }}
  ...
  private renderProviderPanel() {
    if (!this.providerControls.enabled || !this.providerPanelOpen) return nothing;
    const zh = this.locale.toLowerCase().startsWith('zh');
  ...
  private renderProviderEditor() {
    if (!this.providerControls.enabled || !this.providerEditorOpen) return nothing;
    const zh = this.locale.toLowerCase().startsWith('zh');
        ...
        <div class="provider-editor-grid">
          ...
          <label class="provider-editor-field">
            ...
            <input
              ...
              @input=${(event: Event) => {
                this.providerEditorProviderName = (event.target as HTMLInputElement).value;
              }}
            />
          </label>
          <label class="provider-editor-field">
            ...
            <select
              ...
              @change=${(event: Event) => {
                this.providerEditorApiParadigm = (event.target as HTMLSelectElement).value;
              }}
            >
          ...
          <label class="provider-editor-field wide">
            ...
            <input
              ...
              @input=${(event: Event) => {
                this.providerEditorBaseUrl = (event.target as HTMLInputElement).value;
              }}
            />
          </label>
          <label class="provider-editor-field wide">
            ...
            <input
              ...
              @input=${(event: Event) => {
                this.providerEditorApiKey = (event.target as HTMLInputElement).value;
              }}
            />
          </label>
              ...
              <div class="provider-editor-model-row">
                <label class="provider-editor-field">
                  ...
                  <input
                    ...
                    .value=${row.modelId}
                    placeholder="gpt-4.1"
                    @input=${(event: Event) => this.updateProviderEditorModel(row.key, {
                ...
                <label class="provider-editor-field">
                  ...
                  <input
                    ...
                    placeholder="128000"
                    @input=${(event: Event) => this.updateProviderEditorModel(row.key, {
                      contextWindow: this.parseOptionalPositiveInt((event.target as HTMLInputElement).value),
                    })}
                  />
                </label>
                <button
  ...
  private async openProviderEditor(providerUid: number | null = null): Promise<void> {
    this.providerPanelOpen = false;
    this.historyOpen = false;
    ...
    if (providerUid !== null) this.loadProviderEditorDraft(providerUid);
  }

  private resetProviderEditorDraft(providerUid: number | null = null): void {
  ...
  private loadProviderEditorDraft(providerUid: number): void {
    ...
    this.providerEditorApiKey = '';
    const rows = this.providerModelsForProvider(providerUid).map((model) =>
      this.createProviderEditorModelRow(
        model.model_id ?? model.modelId ?? model.model_name ?? model.modelName ?? model.name ?? '',
        model.context_window ?? model.contextWindow ?? null,
        model.uid,
      ),
    );
  ...
  private selectProviderPreset = (event: Event): void => {
  ...
  private addProviderEditorModel(): void {
  ...
  private removeProviderEditorModel(key: string): void {
    if (this.providerEditorModels.length <= 1) return;
    this.providerEditorModels = this.providerEditorModels.filter((row) => row.key !== key);
  }

  private updateProviderEditorModel(
    key: string,
    patch: Partial<Omit<ProviderEditorModelRow, 'key'>>,
  ): void {
  ...
  private providerEditorBuiltinModels(): BuiltinProviderCatalogModel[] {
    ...
    if (!provider) return models.slice(0, 200);
    const prefix = provider.prefix ?? '';
    return models.filter((model) =>
      (model.providerPrefix ?? model.provider_prefix ?? '') === prefix,
    );
  }

  private builtinProviderById(id: string): BuiltinProviderCatalogProvider | undefined {
    if (!id) return undefined;
    return this.builtinProviderCatalog?.providers?.find((provider) => provider.id === id);
  }

  private builtinModelById(id: string): BuiltinProviderCatalogModel | undefined {
    if (!id) return undefined;
    return this.builtinProviderCatalog?.models?.find((model) => model.id === id);
  }

  private providerApiParadigm(provider: BuiltinProviderCatalogProvider): string {
  ...
  private builtinModelContext(model: BuiltinProviderCatalogModel): number | null {
    return model.contextWindow ?? model.context_window ?? null;
  }

  private builtinModelLabel(model: BuiltinProviderCatalogModel): string {
  ...
  private providerEditorModelContext(row: ProviderEditorModelRow, rawValue: string): number {
  ...
  private async refreshProviderDefinitions(): Promise<void> {
  ...
  private submitProviderEditor = (event: SubmitEvent): void => {
    ...
    const enabledModels = this.providerEditorModels
      .map((row) => {
        ...
        while (usedModelUids.has(nextModelUid)) nextModelUid += 1;
        return {
          uid,
          model_id: modelId,
    ...
    const providerRegistration: ProviderRegistration = {
      id: providerUid,
    ...
    void this.configureProviderInput(JSON.stringify(bundle), 'json')
      .then((configured) => {
        if (configured) this.providerEditorOpen = false;
      });
  };

  private existingProviderRegistrationsExcept(providerUid: number): ProviderRegistration[] {
    const providers = this.providerDefinitions?.providers ?? [];
    return providers
  ...
  private providerRegistrationFromDefinition(provider: NonNullable<ProviderDefinitionsResult['providers']>[number]): ProviderRegistration | null {
  ...
  private async configureProviderInput(
    input: string,
    source: 'path' | 'json' | 'text',
  ): Promise<boolean> {
    if (!this.providerControls.enabled) return false;
    this.providerOperation = 'import';
    try {
      ...
      if (this.providerOperation === 'import') this.providerOperation = null;
    }
  }

  private providerModels(): ProviderModelDefinition[] {
    return this.providerDefinitions?.models ?? [];
  }

  private providerModelsForProvider(providerUid: number): ProviderModelDefinition[] {
    return this.providerModels().filter((model) =>
      (model.provider_uid ?? model.providerUid) === providerUid,
    );
  }

  private nextProviderUid(): number {
    return Math.max(
      0,
      ...(this.providerDefinitions?.providers ?? []).map((provider) => provider.uid),
    ) + 1;
  }

  private nextModelUid(used = new Set<number>()): number {
  ...
  private isCurrentArchive(archive: PersistedConversation): boolean {
    ...
    return Boolean(
      (binding?.archiveId && binding.archiveId === archive.archiveId) ||
      (
        !binding?.archiveId &&
  ...
  private formatArchiveTime(value: string | undefined): string {
    if (!value) return '';
  ...
  private emitProviderError(operation: string, error: unknown): void {
    this.emit('agent-conversation-error', {
  ...
  private renderPermissionDialog(displayItems: DisplayItem[]) {
    const permissions = this.state.pendingPermissions.filter(
      (permission) => !this.hasRenderedToolAnchor(permission.tool_call_id, displayItems),
    );
    if (!permissions.length) return nothing;
    const zh = this.locale.toLowerCase().startsWith('zh');
    return html`<div class="permission-backdrop" part="permission-backdrop">
      <section
        class="permission-dialog"
        part="permission-dialog"
        role="dialog"
        aria-modal="true"
        aria-labelledby="tool-permission-heading"
      >
        <h2 id="tool-permission-heading" class="permission-heading">
          ${permissions.length === 1
            ? (zh ? '允许 Agent 执行这个工具？' : 'Allow this tool call?')
            : (zh ? `有 ${permissions.length} 个工具等待确认` : `${permissions.length} tool calls need approval`)}
        </h2>
        <p class="permission-summary">
          ${zh
            ? '这些工具尚未执行。拒绝不会改变外部状态。'
            : 'These tools have not run. Denying them will not change external state.'}
        </p>
        <div class="permission-tools">
          ${permissions.map((permission) => this.renderDialogPermissionItem(permission))}
        </div>
      </section>
    </div>`;
  }

  private renderDialogPermissionItem(permission: PendingToolPermission) {
    const zh = this.locale.toLowerCase().startsWith('zh');
  ...
  private permissionEffectLabel(permission: PendingToolPermission, zh: boolean): string {
  ...
  private permissionArguments(permission: PendingToolPermission): string {
  ...
  private hasRenderedToolAnchor(
    ...
    if (this.hideToolCalls) return false;
    const contentHasAnchor = (content: string | undefined): boolean =>
      Boolean(content?.split(/\r?\n/).some((line) => {
        const match = line.trim().match(/^\[tool:status\s*\|\s*call_id="([^"]+)"\]$/);
        return match?.[1] === toolCallId;
      }));

    return displayItems.some((item) => {
      if (item.kind === 'record') {
        return item.record.role === 'assistant' && contentHasAnchor(displayText(item.record));
      }
      return (
        (item.item.kind === 'assistant-markdown' || item.item.kind === 'notice') &&
        contentHasAnchor(item.item.content ?? '')
      );
    });
  }

  private renderHistoryPanel() {
    if (!this.persistence.enabled || !this.historyOpen) return nothing;
    const zh = this.locale.toLowerCase().startsWith('zh');
  ...
  private renderDisplayItem(
    ...
    if (item.kind === 'presentation') {
      ...
      return html`<article class="message assistant" part="assistant-message">
        <agent-conversation-rich-content
          ...
          .permissionOperation=${this.permissionOperation}
          .colorScheme=${scheme}
          .locale=${this.locale}
          ...
          .widgetsExpired=${true}
          @agent-tool-permission-decision=${this.onToolPermissionDecision}
    ...
    return html`<article class="message assistant" part="assistant-message">
      <agent-conversation-rich-content
        ...
        @agent-content-reveal=${this.followReveal}
        @agent-content-reveal-complete=${() => this.completeReveal(item.key)}
        @agent-widget-submit=${(event: CustomEvent<{ content: string }>) =>
          void this.send(event.detail.content, { source: 'widget' })}
      ></agent-conversation-rich-content>
      ${showDisclaimer
  ...
  private createDisplayItems(): DisplayItem[] {
    const records: DisplayItem[] = this.state.records
      .filter((record) =>
        record.role === 'user' ||
    ...
    const items = [...records];
    const anchorInsertCounts = new Map<string, number>();
    for (const presentation of visiblePresentationItems(
      this.presentationItems,
      this.completedRevealKeys,
    )) {
  ...
  private latestWidgetRecordKey(items: DisplayItem[]): string | null {
  ...
  private latestAssistantKey(items: DisplayItem[]): string | null {
    for (let index = items.length - 1; index >= 0; index -= 1) {
      const item = items[index];
      if (
        item.kind === 'record' &&
  ...
  private handleTransportEvent(event: ConversationTransportEvent): void {
    ...
    if (event.type === 'pending-user-message') {
      ...
      this.dispatch({
        type: 'local-message-added',
        message: createPendingUserMessage(
          event.messageId,
          event.content,
          event.createdAt,
        ),
  ...
  private emitConnectionChange(state: ConversationState['connection']): void {
  ...
  private completeReveal(key: string): void {
    if (this.completedRevealKeys.has(key)) return;
    this.completedRevealKeys.add(key);
    this.requestUpdate();
    this.followReveal();
  }

  private markRecordsRevealComplete(records: readonly LedgerRecord[]): void {
  ...
  private refreshRevealTracking(): void {
  ...
  private statusLabel(): string {
    ...
    if (this.state.connection === 'connecting') return 'Connecting';
  ...
  private configurePersistenceController(): void {
    const controller = this.persistence.enabled
      ? this.persistence.controller
      : null;
    if (controller === this.persistenceController) return;
    this.persistenceUnsubscribe?.();
    this.persistenceUnsubscribe = null;
    this.persistenceController = controller;
    this.persistedConversations = [];
    if (controller?.subscribe) {
      this.persistenceUnsubscribe = controller.subscribe(
        (event) => this.handlePersistenceEvent(event),
      );
    }
  ...
  private handlePersistenceEvent(event: ConversationPersistenceEvent): void {
    if (event.type === 'archive-updated') {
      this.upsertPersistedConversation(event.archive);
    } else if (event.type === 'archive-deleted') {
      this.persistedConversations = this.persistedConversations.filter(
        (archive) => archive.archiveId !== event.archiveId,
      );
  ...
  private upsertPersistedConversation(archive: PersistedConversation): void {
    this.persistedConversations = [
      archive,
      ...this.persistedConversations.filter(
        (item) => item.archiveId !== archive.archiveId,
      ),
    ];
  }
  ...
  private activityLabel(): string | null {
    if (this.state.runtimeState === 'compacting') return '总结中';
    if (this.state.runtimeState === 'waiting') return null;
    if (this.state.turnObservedAssistant) return null;
    return 'Loading';
  }

  private resolvedColorScheme(): 'light' | 'dark' {
