...
describe('AgentRuntimeConversationElement', () => {
  ...
  it('renders a permission request and returns the decision through the transport', async () => {
    ...
    const dialog = element.shadowRoot?.querySelector('[part="permission-dialog"]');
    expect(dialog?.textContent).toContain('Write file');
  ...
  it('renders permission actions inside the matching tool call bubble', async () => {
    const element = new AgentRuntimeConversationElement();
    const transport = new TestTransport();
    element.transport = transport;
    document.body.append(element);
    ...
    expect(element.shadowRoot?.querySelector('[part="permission-dialog"]')).toBeNull();
    ...
    const allow = richContent.shadowRoot.querySelector<HTMLButtonElement>(
      '.tool-permission-actions button:not(.deny)',
    );
    allow?.click();
    await vi.waitFor(() => {
      expect(transport.resolveToolPermission).toHaveBeenCalledWith({
  ...
  it('renders multiple unanchored permission requests in the fallback dialog', async () => {
    const element = new AgentRuntimeConversationElement();
    const transport = new TestTransport();
    element.transport = transport;
    document.body.append(element);
  ...
  it('falls back only for pending permissions without a rendered tool bubble', async () => {
    const element = new AgentRuntimeConversationElement();
    const transport = new TestTransport();
    element.transport = transport;
    document.body.append(element);
    ...
    const dialog = element.shadowRoot?.querySelector('[part="permission-dialog"]');
    expect(dialog?.textContent).toContain('Write file');
    expect(dialog?.textContent).not.toContain('Read file');
    const richContent = element.shadowRoot?.querySelector(
      'agent-conversation-rich-content',
    ) as HTMLElement & { updateComplete: Promise<boolean>; shadowRoot: ShadowRoot };
    await richContent.updateComplete;
  ...
  it('uses the fallback dialog when tool bubbles are intentionally hidden', async () => {
    const element = new AgentRuntimeConversationElement();
    const transport = new TestTransport();
    element.transport = transport;
