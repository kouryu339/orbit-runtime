...
use corework::workflow::nodes::data::{ArrayLengthNode, GetArrayElementNode, MakeArrayNode};
use std::collections::HashMap;

#[test]
fn array_nodes_compose_make_indexed_items_and_length() -> corework::error::Result<()> {
    ...
    first_inputs.insert("Index".to_string(), DataValue::from_i64(0));
    let first_outputs = GetArrayElementNode::new().evaluate(first_inputs)?;
    assert_eq!(
    ...
    last_inputs.insert("Index".to_string(), DataValue::from_i64(-1));
    let last_outputs = GetArrayElementNode::new().evaluate(last_inputs)?;
