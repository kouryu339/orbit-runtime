...
`ToolDescriptor.outputs` 同时也是 workflow 节点的输出 schema。工具作为 workflow
节点成功执行时，`AIOutput.result_json` 必须是包含每个已注册输出字段的 JSON 对象。
Corework 展开 AIOutput envelope，并把声明字段直接暴露为节点引脚。

例如 outputs 声明 `page_id` 与 `url`，应返回：
