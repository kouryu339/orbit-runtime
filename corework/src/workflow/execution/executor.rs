...
use crate::workflow::execution::trace::{bounded_json_preview, data_outputs_to_json};
...
impl BlueprintExecutor {
    ...
    async fn execute_single_node(
        ...
        let result_preview = match &output {
            NodeOutput::Data(outputs) => Some(bounded_json_preview(data_outputs_to_json(outputs))),
            NodeOutput::Loop { iterations, .. } => Some(bounded_json_preview(serde_json::json!({
                "iterations": iterations
                    .iter()
                    .map(|iteration| data_outputs_to_json(&iteration.outputs))
                    .collect::<Vec<_>>()
            }))),
