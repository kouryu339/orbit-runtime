...
impl MinNode {
    ...
    pub fn evaluate(
        ...
        let a = inputs.get("A").and_then(|v| v.as_f64()).ok_or_else(|| {
            crate::error::FrameworkError::SystemError("Min: Invalid input 'A'".to_string())
        })?;

        let b = inputs.get("B").and_then(|v| v.as_f64()).ok_or_else(|| {
            crate::error::FrameworkError::SystemError("Min: Invalid input 'B'".to_string())
        })?;

        let mut outputs = HashMap::new();
        outputs.insert("Result".to_string(), DataValue::from_f64(a.min(b)));
        Ok(outputs)
    }
}
