...
#[register_node(
    ...
    version = "1.0.0",
    category = "Logic",
    display_name = "LessOrEqual",
    description = "→{{IsLessOrEqual}} = {{A}} <= {{B}}",
    permissions = 0,
    data_in = ["A:i64@左操作数", "B:i64@右操作数"],
    data_out = ["IsLessOrEqual:bool@比较结果，A<=B时为true"]
)]
pub struct LessOrEqualNode;

impl Default for LessOrEqualNode {
    fn default() -> Self {
        Self
    }
}

impl LessOrEqualNode {
    pub fn new() -> Self {
        Self
    }
}

impl BlueprintNode for LessOrEqualNode {
    fn name(&self) -> &str {
        "LessOrEqual"
    }
    ...
    fn pins(&self) -> Vec<Pin> {
        vec![
            Pin::data_in("A", "i64"),
            Pin::data_in("B", "i64"),
            Pin::data_out("IsLessOrEqual", "bool"),
        ]
    }
...
impl LessOrEqualNode {
    pub fn evaluate(
        ...
        outputs.insert("IsLessOrEqual".to_string(), DataValue::from_bool(result));
        Ok(outputs)
    }
}
...
mod tests {
    ...
    fn test_less_or_equal_less() {
        ...
        assert_eq!(outputs.get("IsLessOrEqual").unwrap().as_bool(), Some(true));
    }
    ...
    fn test_less_or_equal_false() {
        let node = LessOrEqualNode;
        let mut inputs = HashMap::new();
        inputs.insert("A".to_string(), DataValue::from_i64(7));
        inputs.insert("B".to_string(), DataValue::from_i64(5));

        let outputs = node.evaluate(inputs).unwrap();
        assert_eq!(outputs.get("IsLessOrEqual").unwrap().as_bool(), Some(false));
    }
}
