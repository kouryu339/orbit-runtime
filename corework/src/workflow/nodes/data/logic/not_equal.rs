...
/// NotEqual 节点 - 不相等比较（支持 Any 类型：整数、浮点、字符串、bool）
#[derive(Debug, Clone)]
#[register_node(
    node_type = "Pure",
    version = "1.0.0",
    category = "Logic",
    display_name = "NotEqual",
    description = "→{{IsNotEqual}} = {{A}} != {{B}}",
    permissions = 0,
    data_in = ["A:Any@左操作数", "B:Any@右操作数"],
    data_out = ["IsNotEqual:bool@比较结果，不相等为true"]
)]
pub struct NotEqualNode;

impl Default for NotEqualNode {
    fn default() -> Self {
        Self
    }
}
...
impl BlueprintNode for NotEqualNode {
    fn name(&self) -> &str {
        "NotEqual"
    }
...
impl NotEqualNode {
    pub fn evaluate(
        ...
        outputs.insert("IsNotEqual".to_string(), DataValue::from_bool(result));
        Ok(outputs)
    }
}
...
mod tests {
    ...
    fn test_not_equal_different_values() {
        ...
        assert_eq!(outputs.get("IsNotEqual").unwrap().as_bool(), Some(true));
    }

    #[test]
    fn test_not_equal_false() {
        let node = NotEqualNode::new();
        let mut inputs = HashMap::new();
        inputs.insert("A".to_string(), DataValue::from_i64(5));
        inputs.insert("B".to_string(), DataValue::from_i64(5));

        let outputs = node.evaluate(inputs).unwrap();
        assert_eq!(outputs.get("IsNotEqual").unwrap().as_bool(), Some(false));
    }
}
