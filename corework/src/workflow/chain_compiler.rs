...
impl ChainCompiler {
    ...
    fn first_data_output(node_type: &str) -> Option<String> {
...
impl ChainCompiler {
    ...
    fn compile_node_step(
        ...
        self.nodes.push(BlueprintNodeJson {
            ...
            pins: if pins.is_empty() {
                // 注册表中没有，自动根据 inputs 构建基础引脚
                self.build_impure_pins_from_inputs(inputs, step_id.is_some())
            } else {
                pins
            },
            properties: HashMap::new(),
        ...
        if let Some(sid) = step_id {
            ...
            let out_pin =
                Self::first_data_output(node_type).unwrap_or_else(|| "Result".to_string());
            self.step_map
    ...
    fn compile_inline_pure(&mut self, expr: &InlineExpr) -> ChainResult<(String, String)> {
        ...
        self.nodes.push(BlueprintNodeJson {
            ...
            size: NodeSize::default(),
            pins: if pins.is_empty() {
                self.build_pure_pins_from_inputs(&expr.inputs)
            } else {
                pins
            },
            properties: HashMap::new(),
        ...
        let out_pin = match &expr.output_pin {
            ...
            None => {
                Self::first_data_output(&expr.node_type).unwrap_or_else(|| "Result".to_string())
            }
        };
    ...
    // ── 引脚自动构建（注册表缺失时的 fallback）────────────────────────────

    /// 为 Impure 节点构建最小 pins（有 In/Then + 输入引脚 + 可选输出引脚）
    fn build_impure_pins_from_inputs(
        &self,
        inputs: &[(String, Value)],
        has_output: bool,
    ) -> Vec<NodePin> {
    ...
    /// 为 Pure 节点构建最小 pins（仅 DataInput + DataOutput）
    fn build_pure_pins_from_inputs(&self, inputs: &[(String, Value)]) -> Vec<NodePin> {
