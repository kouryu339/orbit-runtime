...
fn extract_outputs_from_value(
    ...
    let obj = output_value.as_object().ok_or_else(|| {
        FrameworkError::InvalidData(
            "AIOutput.result must be an object containing the registered output fields".to_string(),
        )
    })?;
    for mapping in output_mappings {
        ...
        let field_value = obj.get(field_name).ok_or_else(|| {
            FrameworkError::InvalidData(format!(
                "AIOutput.result is missing registered output field '{field_name}'"
            ))
        })?;
        outputs.insert(
...
mod tests {
    ...
    #[test]
    fn extract_single_named_output_from_result_object() {
        let mappings = vec![PinCacheMapping::new("context", "node:context", "String")];
        let outputs =
            extract_outputs_from_value(&mappings, json!({ "context": "registered field value" }))
                .unwrap();

        assert_eq!(
            outputs.get("context").and_then(DataValue::as_str),
            Some("registered field value")
        );
    }

    #[test]
    fn reject_scalar_result_even_for_single_output() {
        let mappings = vec![PinCacheMapping::new("context", "node:context", "String")];
        let error = extract_outputs_from_value(&mappings, json!("hidden envelope value"))
            .expect_err("result must be expanded by registered field name");

        assert!(error.to_string().contains("registered output fields"));
    }
}
