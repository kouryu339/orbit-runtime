...
pub struct SystemRegistry {
    ...
    >,
    dynamic_metadata:
        Arc<parking_lot::RwLock<HashMap<String, crate::rpc_tool::RuntimeToolMetadata>>>,
}
...
impl SystemRegistry {
    pub fn new() -> Self {
        Self {
            ...
            dynamic_metadata: Arc::new(parking_lot::RwLock::new(HashMap::new())),
        }
    }
    ...
    pub fn register_dynamic(
        ...
        let name = name.into();
        self.dynamic_metadata.write().remove(&name);
        self.dynamic_executors.write().insert(name, executor);
    }

    pub fn register_dynamic_with_metadata(
        &self,
        metadata: crate::rpc_tool::RuntimeToolMetadata,
        executor: Arc<dyn crate::workflow::dynamic_node::DynamicExecute>,
    ) {
        let name = metadata.name.clone();
        self.dynamic_metadata.write().insert(name.clone(), metadata);
        self.dynamic_executors.write().insert(name, executor);
    }

    pub fn get_dynamic_metadata(&self, name: &str) -> Option<crate::rpc_tool::RuntimeToolMetadata> {
        self.dynamic_metadata.read().get(name).cloned()
    }
    ///
    /// ## Example
    ///
