...
use ai_assistant::{AssistantContext, ConversationManager, SkillManager};
...
pub(crate) fn collect_workflow_studio_node_capabilities(
    ...
    let callable_local_tools: HashSet<String> = AssistantContext::all_registered_tool_names()
        .into_iter()
        .collect();
    // `system_only` operations never register NodeMetadata, so this catalog only
    // contains nodes that may be shown in the Workflow Studio graph editor.
    let mut capabilities: Vec<Value> = NodeRegistry::all()
        .into_iter()
        .map(|node| {
            json!({
                "name": node.node_type,
        ...
        .collect();
    capabilities.extend(runtime_tools.iter().map(|tool| {
        json!({
            "name": tool.name,
            "source": "rpc",
            "category": "RPC",
...
pub(crate) fn parse_workflow_studio_options(
    ...
    if options_json.trim().is_empty() {
        return Ok(WorkflowStudioOpenOptions::default());
    }
...
fn handle_workflow_studio_request(
    ...
    match (request.method.as_str(), request.path.as_str()) {
        ...
        ("POST", "/api/compile") => StudioResponse::Json(200, studio_compile_json(&request.body)),
                ...
                events
                    ...
                    .filter(|event| {
                        event
                            .get("conversation_id")
                            .and_then(Value::as_str)
                            .map(|value| value == state.editor_conversation_id)
                            .unwrap_or(false)
                    })
                    .cloned()
                    .collect::<Vec<_>>()
...
fn studio_state_snapshot_response(state: &WorkflowStudioState, body: &[u8]) -> StudioResponse {
    ...
    let snapshot_text = input
        .get("snapshot_text")
        .or_else(|| input.get("snapshotText"))
        .and_then(Value::as_str)
        .unwrap_or("")
        .trim()
        .to_string();
    if snapshot_text.len() > 64 * 1024 {
        return StudioResponse::Json(400, json!({"error": "snapshot_text is too large"}));
    }
    let workflow_snapshot = input
        .get("workflow_snapshot")
        .or_else(|| input.get("workflowSnapshot"))
        .cloned();
    if snapshot_text.is_empty() && workflow_snapshot.is_none() {
        return StudioResponse::Json(
            400,
            json!({"error": "snapshot_text or workflow_snapshot is required"}),
        );
    }

    if !snapshot_text.is_empty() {
        if let Err(e) = set_workflow_studio_current_draft(state, snapshot_text.clone()) {
            return StudioResponse::Json(500, json!({"error": e.to_string()}));
        }
    }
    let workflow_snapshot_bytes = if let Some(snapshot) = workflow_snapshot {
        let snapshot = match serde_json::to_string(&snapshot) {
            Ok(value) => value,
            Err(e) => {
                return StudioResponse::Json(
                    400,
                    json!({"error": format!("workflow_snapshot serialize failed: {e}")}),
                );
            }
        };
        if snapshot.len() > 64 * 1024 {
            return StudioResponse::Json(400, json!({"error": "workflow_snapshot is too large"}));
        }
        if let Err(e) =
            set_workflow_studio_snapshot_field(state, "workflow_studio.workflows", snapshot.clone())
        {
            return StudioResponse::Json(500, json!({"error": e.to_string()}));
        }
        snapshot.len()
    ...
    StudioResponse::Json(
        200,
        json!({
            "schema": "workflow-studio-state-snapshot-result/v1",
            "fields": {
                "workflow_studio.current_draft": snapshot_text.len(),
                "workflow_studio.workflows": workflow_snapshot_bytes
            }
        }),
    )
}
...
fn set_workflow_studio_workflows_snapshot(
    state: &WorkflowStudioState,
    snapshot: Value,
) -> Result<(), RuntimeError> {
    let snapshot =
        serde_json::to_string(&snapshot).map_err(|e| RuntimeError::Internal(e.to_string()))?;
    set_workflow_studio_snapshot_field(state, "workflow_studio.workflows", snapshot)
}
...
fn workflow_snapshot_from_decompile_input(input: &Value) -> Option<Value> {
    input
        .get("workflow_snapshot")
        .or_else(|| input.get("workflowSnapshot"))
        .cloned()
...
fn maybe_set_workflow_snapshot_from_input(
    state: &WorkflowStudioState,
    input: &Value,
) -> Result<(), RuntimeError> {
    if let Some(snapshot) = workflow_snapshot_from_decompile_input(input) {
        set_workflow_studio_workflows_snapshot(state, snapshot)?;
    }
    Ok(())
}
...
fn studio_workflows_json(state: &WorkflowStudioState) -> Value {
    ...
    if let Ok(entries) = fs::read_dir(state.workflows.workflows_dir()) {
        for entry in entries.flatten() {
            let path = entry.path();
            if !corework::workflow::blueprint_json::BlueprintJson::is_workflow_file_path(&path) {
                continue;
            }
            if let Ok(blueprint) =
                corework::workflow::blueprint_json::BlueprintJson::from_workflow_file(&path)
            {
                workflows.push(json!({
                    "file_name": path.file_name().and_then(|v| v.to_str()).unwrap_or(""),
    ...
    json!({
        "schema": "workflow-studio-workflows/v1",
        "workflows_dir": state.workflows.workflows_dir().to_string_lossy(),
        "workflows": workflows
    })
}

fn studio_workflow_detail_json(state: &WorkflowStudioState, name: &str) -> Value {
    match find_workflow_file(state.workflows.workflows_dir(), name) {
        Some(path) => {
            match corework::workflow::blueprint_json::BlueprintJson::from_workflow_file(&path) {
                Ok(blueprint) => json!({
                    "schema": "workflow-studio-workflow-detail/v1",
                    "file_name": path.file_name().and_then(|v| v.to_str()).unwrap_or(""),
                    "path": path.to_string_lossy(),
                    "blueprint": blueprint
                }),
        ...
        None => json!({"error": "workflow not found"}),
    }
}

fn studio_compile_json(body: &[u8]) -> Value {
    let input: Value = match serde_json::from_slice(body) {
        Ok(value) => value,
    ...
    match corework::workflow::chain_compiler_v2::compile_chain_v2(script) {
        Ok(blueprint) => json!({
...
fn studio_decompile_response(state: &WorkflowStudioState, body: &[u8]) -> StudioResponse {
    let input: Value = match serde_json::from_slice(body) {
        Ok(value) => value,
        Err(e) => {
            return StudioResponse::Json(400, json!({"error": format!("invalid JSON body: {e}")}));
    ...
    if let Err(e) = set_workflow_studio_current_draft(state, script.clone()) {
        return StudioResponse::Json(500, json!({"error": e.to_string()}));
    }
    if let Err(e) = maybe_set_workflow_snapshot_from_input(state, &input) {
        return StudioResponse::Json(500, json!({"error": e.to_string()}));
    }
    StudioResponse::Json(
        200,
        json!({
            ...
            "field": "workflow_studio.current_draft",
            "bytes": script.len(),
            "script": script
        }),
    )
...
fn studio_normalize_json(state: &WorkflowStudioState, body: &[u8]) -> Value {
    ...
    let mut normalized = match corework::workflow::chain_compiler_v2::compile_chain_v2(&script) {
        Ok(value) => value,
        Err(e) => {
            return json!({
...
fn studio_save_json(state: &WorkflowStudioState, body: &[u8]) -> Value {
    ...
    let blueprint_value = input.get("blueprint").cloned().unwrap_or(input);
    let blueprint =
        match corework::workflow::blueprint_json::BlueprintJson::from_json_value(blueprint_value) {
            Ok(value) => value,
            Err(e) => return json!({"error": format!("invalid BlueprintJson: {e}")}),
        };
    match state.workflows.save_local(&blueprint) {
        Ok(key) => {
            let _ = state.workflows.scan_local_dir();
            json!({
                "schema": "workflow-studio-save-result/v1",
                "key": key,
...
fn studio_run_json(state: &WorkflowStudioState, body: &[u8]) -> Value {
    ...
    let result = run_short_tokio(async move {
        workflows
            .execute_from_blueprint_report(blueprint, inputs, trace)
            .await
    });
    match result {
        Ok(report) => {
            ...
            json!({
                "schema": "workflow-studio-run-result/v1",
                "outputs": outputs,
                "trace": report.trace
            })
        }
...
fn find_workflow_file(workflows_dir: &Path, name: &str) -> Option<PathBuf> {
    ...
    for entry in entries.flatten() {
        ...
        if let Ok(blueprint) =
            corework::workflow::blueprint_json::BlueprintJson::from_workflow_file(&path)
        {
            if blueprint.metadata.name == name || blueprint.metadata.id == name {
                return Some(path);
            }
        }
    }
...
start -> end</textarea>
  ...
  <script>
    ...
    function renderRegistry() {
      ...
      const workflows = state.workflows.filter(w => `${w.metadata?.name} ${w.file_name}`.toLowerCase().includes(q));
      $('workflows').innerHTML = workflows.map(w => `<button class="item" data-workflow="${escapeHtml(w.file_name)}"><span class="item-title">${escapeHtml(w.metadata?.name || w.file_name)}</span><span class="item-meta">${w.nodes} nodes · ${w.connections} wires</span></button>`).join('') || '<span class="item-meta">No saved workflows</span>';
    ...
    async function openWorkflow(name) {
      try { const out = await api('/api/workflows/' + encodeURIComponent(name)); setBlueprint(out.blueprint); setOutput(out); } catch (error) { setOutput({ error: error.message }); }
    }
    ...
    function applyRuntimeEvent(event) {
      ...
      if (event.type === 'workflow_studio:draft_update') {
        const payload = event.payload || {};
        if (!payload.blueprint || typeof payload.script !== 'string') return;
        state.blueprint = payload.blueprint;
        $('script').value = payload.script;
        renderGraph();
        scheduleStudioSnapshot(payload.replace_draft ? 'agent_open_draft' : 'agent_draft');
        appendMessage('system', 'Applied AI draft.', `draft_${payload.revision || Date.now()}`);
        return;
      }
      const payload = event.payload || {};
      if (event.type === 'frontend:state_snapshot') {
