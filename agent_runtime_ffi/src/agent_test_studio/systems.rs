...
#[define_operation(
    ...
    display_name = "创建身份{identity}、人格{personality}、背景{background}、目标{goal}、策略{strategy}、隐藏事实{hidden_facts}和边界{boundaries}的对抗测试，并发送{initial_message}",
...
#[define_operation(
    ...
    display_name = "因{reason}销毁对抗测试{pair_id}",
    category = "Agent Test Studio",
...
#[define_operation(
    ...
    display_name = "按{mode}检查对抗测试{pair_id}",
    category = "Agent Test Studio",
...
#[define_operation(
    ...
    display_name = "以摘要{summary}结束对抗测试，记录发现{finding_title}、观察{finding_observation}、预期{finding_expected_behavior}和证据轮次{evidence_turns}",
    category = "Agent Test Studio",
