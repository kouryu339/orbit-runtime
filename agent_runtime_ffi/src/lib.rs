...
fn invoke_command(
    ...
    let result = match command_type {
        ...
        "workflow.get_reference_node" => facade.workflow_reference_node(
            &required_string(payload, "workflow_id")?,
            &required_string(payload, "node_id")?,
        )?,
        ...
        "workflow.describe_inputs" => facade.describe_workflow_inputs(
            payload.get("id").and_then(Value::as_str),
            payload.get("script").and_then(Value::as_str),
        )?,
        ...
        "workflow.start" => facade.start_workflow_run()?,
        "workflow.run" => {
            let execution_context = optional_workflow_execution_context(payload)?;
            facade.run_workflow(
                required_string(payload, "workflow_run_id")?,
                payload
                    .get("id")
                    .and_then(Value::as_str)
                    .map(str::to_string),
        ...
        "conversation.set_model" => {
            ...
            let model_uid = payload
                ...
                .and_then(|value| u32::try_from(value).ok())
                .ok_or_else(|| FfiError::invalid_argument("payload.model_uid must be a uint32"))?;
            let admission = facade.set_conversation_model_with_admission(
                &conversation_id,
                model_uid,
                command_id.to_owned(),
            )?;
...
pub extern "C" fn agent_runtime_capabilities_v1() -> *const c_char {
    ...
    CAPABILITIES
        .get_or_init(|| {
            ...
            .expect("static capabilities JSON must parse");
...
mod abi_tests {
    ...
    fn abi_metadata_is_self_describing() {
        ...
        assert_eq!(
            value["events"]["lines"]["workflow_run"]["aggregate_id"],
        ...
        let state_ops = value["state"]["ops"].as_array().unwrap();
        assert!(state_ops.iter().any(|op| op == "agent.upsert"));
        assert!(state_ops.iter().any(|op| op == "agent.retired"));
        assert!(!event_types
            .iter()
