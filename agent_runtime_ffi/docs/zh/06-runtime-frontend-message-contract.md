...
| `gateway_message` + `metadata.subtype = tool_call_permission_requested` | 工具尚未执行，正在等待用户确认。如果 `frontend:state_snapshot.payload.pending_permissions` 里有相同 `tool_call_id`，允许/拒绝按钮应直接渲染在这个工具控件内。 |
