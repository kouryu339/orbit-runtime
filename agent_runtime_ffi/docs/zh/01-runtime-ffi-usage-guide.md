...
| `runtime.get_tool_definitions` | 空 payload；`start` 后 | 已注册的本地与 RPC 工具定义 |
...
| `workflow.update` | `resource`, `expected_revision?` | 更新后的 workflow |
...
两个 Studio 命令的 `options.agent_id` 都接受 Runtime config agent id、已注册的
cluster agent id（包括 cluster 的 `focus_agent_id`），以及只绑定到一个 cluster agent
的 profile id。空值继续使用默认 Agent。标识不存在或存在歧义时，Runtime 会在关闭已有
Studio 会话之前立即返回 `InvalidConfig`。

`runtime.get_tool_definitions` 返回 `agent-runtime-tool-definitions/v1`。每个条目包含名称、
显示名、描述、输入参数、输出字段、行为标记、所需 capability、Workflow 节点信息，以及
RPC 工具的 transport 标识。`catalog_scope: "runtime_registered"` 表示这是 Runtime 注册
总目录；`authorization_scope: "definitions_only"` 表示它不代表某个 conversation 已激活或
已批准这些工具。该命令只能在 `start` 后调用，因为 RPC `ListTools` 发现必须先完成。

`runtime.get_workflow_node_definitions` 返回
`agent-runtime-workflow-node-definitions/v1`。每个节点包含 `node_type`、`source`、
`display_name`、规范分类和原始分类、`pure` 以及完整 pins。Pure 节点的显示名是值模板，
其中每个 `{field}` 都引用真实引脚；宿主可以替换连接值，将数据链折叠成可读表达式。

`runtime.get_agent_cluster_definitions` 返回
`agent-runtime-agent-cluster-definitions/v1`，包括解析后的 Cluster/Agent 标识、profile
绑定、role、feature、模型选择、retrieval、权限策略，以及注册或内置来源。它不会返回
完整 system prompt 内容。注册 Cluster 可在启动前读取；内置 Studio Cluster 在 `start`
后出现在目录中。

`runtime.get_rpc_endpoint_definitions` 返回
`agent-runtime-rpc-endpoint-definitions/v1`，包括 Endpoint 标识、协议、生命周期归属、
超时、用途、工具名称和启动验证状态。它不会返回 address、启动命令与参数、环境变量、
token 或 header。`health_scope: "startup_only"` 表示它不是持续健康探针：`ready` 表示
启动时 `ListTools` 发现成功，`configured` 表示连接延迟到实际调用时建立。
