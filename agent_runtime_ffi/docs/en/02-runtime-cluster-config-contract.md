...
The cluster policy is the conversation default. A trusted host may override
individual categories for one conversation in
`agent-runtime-conversation-spawn/v1`; omitted categories still inherit the
cluster value:
...
The effective policy is fixed when the conversation is spawned. This override
is a host control and must not be exposed as a model-controlled tool argument.
