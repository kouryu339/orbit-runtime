...
| `gateway_message` with `metadata.subtype = tool_call_permission_requested` | The tool has not run yet and is waiting for a user decision. Keep the matching tool control in `waiting_permission`; render decisions from canonical `pending_permissions` in the approval shelf above the composer. |
