...
function buildWorkflowSnapshot(
  ...
  const items: WorkflowSnapshotItem[] = workflows.map((workflow) => {
    return {
      name: workflow.metadata?.name || workflow.file_name,
...
export function StudioApp() {
  ...
  const decompileDraft = useCallback(async (next: BlueprintJson, reason: string) => {
  ...
  const scheduleSnapshot = useCallback((next: BlueprintJson, reason: string) => {
    if (snapshotTimer.current != null) window.clearTimeout(snapshotTimer.current);
    snapshotTimer.current = window.setTimeout(() => {
      snapshotTimer.current = null;
      decompileDraft(next, reason).catch((error) => setOutput(`Decompile failed: ${error.message}`));
    }, 350);
  }, [decompileDraft]);
  ...
  useEffect(() => {
    ...
    source.onmessage = (message) => {
      ...
      try {
        ...
        if (event.type === 'workflow_studio:draft_update') {
          const payload = event.payload as DraftUpdatePayload | undefined;
          if (!payload?.blueprint || typeof payload.script !== 'string') {
            setOutput('Ignored invalid Workflow Studio draft update event.');
            return;
          }
    ...
    return () => source.close();
  }, [appendSseTrace, applyDraftUpdate]);
  ...
  const refreshWorkflows = useCallback(async () => {
    ...
    await publishWorkflowSnapshot();
  }, [publishWorkflowSnapshot, setWorkflowState]);

  const saveWorkflow = useCallback(async () => {
    if (!blueprint) return;
    const result = await api<{ key: string }>('/api/save', { method: 'POST', body: { blueprint } });
    await refreshWorkflows();
    setDraftOrigin({
      kind: 'workflow_copy',
      file_name: `${result.key}.workflow.json`,
      workflow_name: blueprint.metadata?.name,
    });
    await publishWorkflowSnapshot(blueprint);
    setOutput(`Saved ${result.key}`);
  }, [blueprint, publishWorkflowSnapshot, refreshWorkflows, setDraftOrigin]);

  const testWorkflow = useCallback(async () => {
    if (!blueprint) return;
    const result = await api<unknown>('/api/run', { method: 'POST', body: { blueprint, inputs: {}, trace: true } });
    await api('/api/chat', {
      method: 'POST',
      body: {
          ...
          {rightView === 'chat' ? (
            <div className="studio-chat">
              <StudioConversation
                ...
                beforeSend={async () => {
                  const current = blueprintRef.current;
                  if (current) await decompileDraft(current, 'before_chat');
                }}
