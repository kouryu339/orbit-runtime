...
`executeWorkflowScript` 临时执行工具。持久化 Workflow 工具仍由 role/feature Skill 开放。
...
避免 focus 歧义。
具体实例的 `systemSkills` 按键覆盖 profile 默认值，适合只对集群中的部分 Agent
开放高级工具规则与一次性多行 Workflow 脚本执行能力。Draft/Registered 目录权限必须
由宿主另行配置 role/feature Skill。

Cluster 的 `permissions` 是 conversation 默认值。可信宿主可以在创建单个 conversation
时按分类覆盖；未填写的分类继续使用 cluster 值：

```python
conversation = runtime.invoke("conversation.spawn", {
    "cluster_id": "product-instance",
    "permissions": {"destructive": "ask"},
})
```

模式包括 `full`（直接执行）、`ask`（等待用户批准）和 `deny`（直接拒绝，不创建审批
请求）。不要把 spawn 覆盖参数暴露成模型可控制的参数。
