...
- 把 `frontend:state_snapshot.payload.pending_permissions` 统一渲染在输入框正上方的
  approval shelf；工具气泡只保留等待/运行/完成状态；
- 用户允许或拒绝后，经宿主调用 `conversation.resolve_tool_permission`；这是反向
  command，不是 event，也不是向 SSE 写数据；
...
宿主转发前必须校验 conversation id 的访问权限。过期或重复决定返回
`resolved:false`，不能把它当成工具刚刚执行。
