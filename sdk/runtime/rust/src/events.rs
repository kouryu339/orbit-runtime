...
pub fn workflow_id_from_event(event: &Value) -> Option<String> {
    event
        .pointer("/payload/workflow_id")
        .and_then(Value::as_str)
        .filter(|value| !value.trim().is_empty())
        .map(str::to_string)
}
...
pub fn is_workflow_event(event: &Value) -> bool {
    event
        .get("event_line")
        .or_else(|| event.pointer("/payload/event_line"))
        .and_then(Value::as_str)
        == Some("workflow")
}
...
mod tests {
    ...
    fn public_event_filter_excludes_telemetry_mirrors() {
        ...
        assert!(is_public_runtime_event(&json!({
            "type": "workflow.resource_changed",
            "payload": {"operation": "registered"}
        })));
        assert!(!is_public_runtime_event(&json!({
            "type": "llm_usage",
            "conversation_id": "conv_1",
    ...
    #[test]
    fn workflow_subscriptions_are_isolated_from_conversation_events() {
        let bus = RuntimeEventBus::new();
        let workflow = bus.subscribe_workflow("wf_1");
        let conversation = bus.subscribe_conversation("conv_1");
        ...
        assert_eq!(
            workflow.try_recv().unwrap()["payload"]["workflow_id"],
            "wf_1"
        );
        assert!(matches!(
