...
The SDK is aligned with Orbit Runtime native release `v0.4.7-beta.1` / ABI 1.3. Use
...
conversation with the typed spawn API:

```go
conversation, err := runtime.SpawnConversation(ctx, runtimehost.ConversationSpawnOptions{
    ClusterID: "assistant",
    Permissions: &runtimehost.ToolPermissionPolicy{
        Destructive: "ask",
    },
})
```
...
`runtime.ResolveToolPermission(ctx, conversationID, toolCallID, "allow")` or
`"deny"`.

After startup, use `CreateWorkflowDraft`, `ReadWorkflow`,
`RegisterWorkflowDraft`, `UpdateWorkflow`, `CompileWorkflowDraft`,
...
After startup, `ToolDefinitions` returns the unified registered local/RPC tool
catalog. It is definition discovery, not per-conversation activation or
authorization state.

`WorkflowNodeDefinitions` returns the shared Corework/local/RPC graph-node
catalog, including Pure-node `display_name` templates and complete pins.

`AgentClusterDefinitions` returns effective registered and built-in Agent
topology without prompt content. `RPCEndpointDefinitions` returns sanitized
