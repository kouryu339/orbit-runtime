...
conversation by setting `ConversationSpawnOptions::permissions_json`:

```cpp
ConversationSpawnOptions spawn;
spawn.cluster_id = "assistant";
spawn.permissions_json = R"({"destructive":"ask"})";
auto conversation = runtime.spawn_conversation(spawn);
```
...
`runtime.resolve_tool_permission(conversation_id, tool_call_id, "allow")` or
`"deny"`.
...
- dynamic Draft create/read/compile/promotion, resource update/delete/list,
...
Use `workflow_script_to_blueprint` and `workflow_blueprint_to_script` for
stateless conversion; these calls do not change revisions or emit Workflow
events.
...
This header targets Orbit Runtime native release `v0.4.6-beta.1` / ABI 1.3. Download the
platform package from:

```text
https://github.com/kouryu339/orbit-runtime/releases/tag/v0.4.6-beta.1
```
