...
describe('AgentRuntimeConversationElement', () => {
  ...
  it('renders a permission request and returns the decision through the transport', async () => {
    ...
    expect(shelf?.textContent).toContain('report.md');
    expect(shelf?.textContent).not.toContain('private document body');
    expect(shelf?.textContent).not.toContain('do-not-render');
    expect(shelf?.querySelector('pre')).toBeNull();
