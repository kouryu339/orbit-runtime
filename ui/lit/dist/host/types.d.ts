import type { ConversationConnectionState, ConversationState, FrontendSnapshotPayload } from '../protocol/types.js';
export declare const HOST_CONTRACT: "agent-conversation-host/v1";
export declare const TRANSPORT_CONTRACT: "agent-conversation-transport/v1";
export declare const PRESENTATION_CONTRACT: "agent-conversation-presentation/v1";
export declare const EXTENSION_CONTRACT: "agent-conversation-extension/v1";
...
export type ConversationTransportHandlers = {
    ...
    connection(state: Exclude<ConversationConnectionState, 'connecting'>): void;
...
export type ImportImageRequest = ConversationCommandRequest & {
    file: File;
};
...
export type PresentationItemPatch = Partial<Omit<ConversationPresentationItem, 'contract' | 'id'>>;
...
export type ConversationThemeName = ExtensibleString<'studio' | 'paper' | 'soft' | 'midnight' | 'neutral' | 'blue' | 'green' | 'amber'>;
export type ConversationDensity = 'compact' | 'comfortable';
export declare const PERSISTENCE_CONTRACT: "agent-conversation-persistence/v1";
export type PersistedConversationStatus = 'sealed' | 'running' | 'saving' | 'restoring' | 'error';
export type ConversationInstanceState = 'current' | 'background' | 'data_only';
...
export interface ConversationPersistenceController {
    ...
    subscribe?(listener: (event: ConversationPersistenceEvent) => void): () => void;
}
