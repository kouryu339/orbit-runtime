export const TOOL_PLACEHOLDER_PATTERN = /\[tool:status\s*\|\s*call_id="([^"]+)"\]/g;
...
export function displayText(record) {
 ...
 */
export function displayTextWithToolAnchors(record) {
    const content = displayText(record);
    if (record.role !== 'assistant')
        return content;
    const embedded = new Set(Array.from(content.matchAll(TOOL_PLACEHOLDER_PATTERN), (match) => match[1]));
...
export function snapshotRecords(payload) {
    if (payload.ledger_records)
        return payload.ledger_records;
    if (payload.ledger)
        return payload.ledger;
    if (payload.ledger_delta?.records)
        return payload.ledger_delta.records;
    if (payload.ledger_delta?.record)
        return [payload.ledger_delta.record];
    return [];
}
...
export function toolCallIdsFromRecord(record) {
    const ids = Array.from(displayText(record).matchAll(TOOL_PLACEHOLDER_PATTERN), (match) => match[1]);
    ...
    if (Array.isArray(declarations)) {
        for (const call of declarations) {
            if (call && typeof call === 'object' && 'id' in call && typeof call.id === 'string' && !ids.includes(call.id))
                ids.push(call.id);
        }
    }
    return ids;
}
export function toolCallFromRecord(record) {
    const subtype = record.metadata?.subtype;
    if (!subtype?.startsWith('tool_call_'))
        return null;
...
export function collectToolCalls(records, previous = []) {
...
export function isRenderableAssistantRecord(record) {
    if (record.role !== 'assistant')
        return false;
